...
class SubjectRoster:
    ...
    def parseCourses(self) -> list:
        self.courses = self.soup.find_all("div", class_="node", role="region")
        return self.courses
    ...
    def findSectionNumberFromClass(self, course: BeautifulSoup) -> str:
        if (
            course.find("li", class_="class-numbers")
            and course.find("li", class_="class-numbers").find("p")
            and course.find("li", class_="class-numbers")
            .find("p")
            .find("strong", class_="tooltip-iws")
        ):
            return (
                course.find("li", class_="class-numbers")
                .find("p")
                .find("strong", class_="tooltip-iws")["data-content"]
            )
        else:
            return "None"

    def findProfFromClass(self, course: BeautifulSoup) -> str:
        if course.find("li", class_="instructors") and course.find(
            "li", class_="instructors"
        ).find("span", class_="tooltip-iws"):
            return course.find("li", class_="instructors").find(
                "span", class_="tooltip-iws"
            )["data-content"]
        else:
            return "None"

    def findDatesFromClass(self, course: BeautifulSoup) -> str:
        if course.find("li", class_="section-alt section-alt-details date-range"):
            return (
                course.find("li", class_="section-alt section-alt-details date-range")
                .contents[0]
                .strip()
            )
    ...
    def findDaysFromClass(self, course: BeautifulSoup) -> str:
        if (
            course.find("li", class_="meeting-pattern")
            and course.find("li", class_="meeting-pattern").find(
                "span", class_="pattern-only"
            )
            and course.find("li", class_="meeting-pattern")
            .find("span", class_="pattern-only")
            .find("span", class_="tooltip-iws")
        ):
            return (
                course.find("li", class_="meeting-pattern")
                .find("span", class_="pattern-only")
                .find("span", class_="tooltip-iws")["data-content"]
            )
        else:
            return "None"
